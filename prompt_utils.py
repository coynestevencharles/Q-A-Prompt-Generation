...
def add_prompt(question: str, lang: str, N: int=3) -> str:
    ...
    #start the prompt:
    if lang == 'en':
        header = 'Answer these questions:\n'
    elif lang == 'ja':
        header = '質問を答えてください：\n'  
    else:
        assert 0, lang
    
    #get example questions and answers put them into Q&A format:
    similar_examples = get_similar(question, lang, N)
    similar_prompts = [subprompt(example, lang) for example in similar_examples]
